...
";
    ...
    for i in 0..row_count {
        for j in 0..(col_count - run_count) {
            let product = get_run_product(&|r| grid[i][j + r]);
            max_product = cmp::max(max_product, product);
        }
    }
    for i in 0..(row_count - run_count) {
        for j in 0..col_count {
            let product = get_run_product(&|r| grid[i + r][j]);
            max_product = cmp::max(max_product, product);
        }
    }
    for i in 0..(row_count - run_count) {
        for j in 0..(col_count - run_count) {
            let product = get_run_product(&|r| grid[i + r][j + r]);
            max_product = cmp::max(max_product, product);
        }
    }
    for i in (run_count - 1)..row_count {
        for j in 0..(col_count - run_count) {
            let product = get_run_product(&|r| grid[i - r][j + r]);
            max_product = cmp::max(max_product, product);
        }
